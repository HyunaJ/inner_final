...
def split_df(df):
    ...
    for index, row in df.iterrows():
        if index in long_text_indices:
            ...
            while parts:
                ...
                if len(current_part) + len(next_part) <= 600:
    ...
    # 250자 이하 텍스트를 합치는 코드
    short_text_indices = new_df2[new_df2['text'].str.len() <=250].index
...
pdf_folder = '/data03/hyunaz/project/inner_ft/law_file/규정문서'
...
embedding_model = SentenceTransformer("/data03/hyunaz/project/inner_ft/ko-sroberta-multitask")  #Your embedding model
#embedding_model = SentenceTransformer("/data03/hyunaz/project/inner_ft/klue-sroberta-base-continue-learning-by-mnr")
...
# 원본 DataFrame에 임베딩 DataFrame을 결합
df = pd.concat([df.reset_index(drop=True), embed_df], axis=1)



# # embedding 저장하기
with open('embeddingsn.pkl', 'wb') as f:
     pickle.dump(df, f)

# 불러오기 
#with open('gpt/정제파일/embedding_Update_0523.pkl', 'rb') as f:
    #df2 = pickle.load(f)
#df2   
...
### bm25 ###
#######################################################################################################
def tokenizer_K(sent):
  #sent = okt.morphs(sent, norm=True, stem=True)
  sent = komoran.morphs(sent)
  return sent

embeddings = [tokenizer_K(doc) for doc in df.iloc[:,0]]
bm25 = BM25Okapi(embeddings)

# 저장
with open('bm25_modeln.pkl', 'wb') as f:
     pickle.dump(bm25, f)
